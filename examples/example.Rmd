---
title: "Example"
author: "Maciej Pacula"
date: '`r Sys.Date()`'
output: html_document

---

```{r setup, include=TRUE}
library(gofigR)
library(ggplot2)
library(plotly)
library(evaluate)
library(stats)
library(gplots)


knitr::opts_chunk$set(echo = TRUE)
enable(analysis_name="My first Rmd analysis 2",
       watermark = gofigR::QR_WATERMARK,
       debug=TRUE)
```

```{r html_test}
func <- function() {
  knitr::asis_output("<img src='https://gofigr.io/wp-content/uploads/2023/03/cropped-GoFigr-500-%C3%97-200-px-1500-%C3%97-340-px-1-2-300x78.png'/><img src='https://gofigr.io/wp-content/uploads/2023/03/cropped-GoFigr-500-%C3%97-200-px-1500-%C3%97-340-px-1-2-300x78.png'/>")
  #knitr::asis_output("https://gofigr.io/wp-content/uploads/2023/03/cropped-GoFigr-500-%C3%97-200-px-1500-%C3%97-340-px-1-2-300x78.png")
}

func()
```

## R Markdown

This is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like this:


## Including Plots

You can also embed plots, for example:


```{r include_image}
gofigR::capture({
  base::plot(pressure)
  base::plot(pressure, main="Another one")
}, pressure)
```


```{r pressure, gofigr_figure_name="My first figure", fig.width=10, fig.height=5, gofigr_on=TRUE}
myplot <- function() {
  res <- NULL
  res <- gofigR::capture({
    plot(pressure, main="Pressure vs temperature")
    text(50, 50, "My pretty figure")
  }, NULL)
  print("Done!")
  print(res)
}

myplot()
```

Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.

```{r barplot, gofigr_on=FALSE}
temperatures <- c(22, 27, 26, 24, 23, 26, 28)

# bar plot of temperatures vector
barplot(temperatures)
```

```{r ggplot}
gp <- ggplot(data=pressure, mapping=aes(x=temperature, y=pressure))
gp <- gp + geom_line()
plot(gp)
```
```{r heatmap}
mat <- matrix(rnorm(1200), ncol=6)

# heatmap with the defaults parameters
heatmap.2(x=mat)
```


```{r plotly}
library(plotly)

fig <- plot_ly(data = iris, x = ~Sepal.Length, y = ~Petal.Length)

print(fig)
```
